...
def crawl_starting_from(url):
    ...
    with open("links_dailycaller.txt", "w") as file:
            ...
            if normalized_url not in visited:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if ("dailycaller" in current_url) and ("app" not in current_url) and (title != '|The Daily Caller'):
                            # file.write(f"{current_url.strip()} {title.strip().splitlines()[0]}\n")
                            file.write(f"{current_url.strip()} ")
                            for item in title.strip().splitlines():
...
# Start crawling from Daily Caller
crawl_starting_from("https://www.dailycaller.com/")
