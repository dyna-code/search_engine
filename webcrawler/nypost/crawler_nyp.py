...
def get_hrefs(link):
    try:
        ...
        hrefs = [[i[0], i[1].replace("\t", "")] for i in hrefs]
        hrefs = [i for i in hrefs if len(i[1]) > 0]
...
f = []
if os.path.isfile('links_nypost.txt'):
        with open('links_nypost.txt', 'r') as file:
            f = file.readlines()
...
while url_queue and len(master_list) < 500:
    ...
    for i in hrefs:
        ...
        if is_valid_domain(i[0]):
            # print('here')
            i[0] = normalize_link(i[0])
            pattern = r"https:\/\/nypost\.com\/(\d{4})\/(\d{2})\/(\d{2})\/([a-zA-Z0-9\-]+)\/([a-zA-Z0-9\-]+)"
            matches = re.findall(pattern, i[0])
            if matches:
                new_link = matches[0][0]
                new_title = ''.join(i[1:])
                if [i[0], new_title] not in master_list:
                    master_list.append([i[0], new_title])
                    url_queue.append([i[0], new_title])
